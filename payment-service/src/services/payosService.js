...
const RETURN_URL =
  process.env.PAYOS_RETURN_URL || "http://localhost:3333/payment-result";
const WEBHOOK_URL =
  process.env.PAYOS_WEBHOOK_URL ||
  "http://localhost:8989/api/payments/payos/webhook";
...
const createPaymentRequest = async (orderInfo) => {
  try {
    ...
    if (
      typeof orderInfo.orderCode === "string" &&
      orderInfo.orderCode.startsWith("ORDER-")
    ) {
    ...
    const paymentData = {
      ...
      description:
        orderInfo.description || `Thanh toán đơn hàng ${orderInfo.orderCode}`,
...
const checkPaymentStatus = async (orderCode) => {
  try {
    const response = await axios.get(
      `${API_URL}/v2/payment-requests/${orderCode}`,
      {
        headers: {
          "x-client-id": CLIENT_ID,
          "x-api-key": API_KEY,
...
const cancelPayment = async (
  ...
  try {
    ...
    const response = await axios.post(
      `${API_URL}/v2/cancel-payment-request`,
      data,
      {
        headers: {
          "x-client-id": CLIENT_ID,
