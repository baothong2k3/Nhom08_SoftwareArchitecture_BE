  ...
  try {
    ...
    if (paymentMethod !== "COD" && paymentMethod !== "BANK") {
      return res.status(400).json({
        success: false,
        message:
          "Phương thức thanh toán không tồn tại. Chỉ hỗ trợ COD hoặc BANK",
      });
    }
    ...
    if (paymentMethod === "COD") {
      try {
        ...
        return res.status(200).json({
          success: true,
          message:
            "Đặt hàng thành công với phương thức thanh toán khi nhận hàng",
          data: {
            orderCode: finalOrderCode,
    ...
    // Nếu là thanh toán qua ngân hàng (BANK)
      ...
      try {
        ...
        console.log(
          "Gửi yêu cầu thanh toán với dữ liệu:",
        ...
        if (paymentResult.code && paymentResult.code !== "00") {
          ...
          return res.status(400).json({
            ...
            message: `Không thể tạo yêu cầu thanh toán: ${
              paymentResult.desc || "Lỗi không xác định"
            }`,
            error: paymentResult,
          });
        }
...
exports.handlePaymentResult = async (req, res) => {
  try {
    ...
    if (paymentStatus === "PAID") {
      ...
      try {
        await axios.post(
          process.env.ORDER_SERVICE_URL + "/api/orders/payment-completed",
          {
            orderCode: orderCode,
          }
        );
        ...
        if (transaction.userId) {
          await axios.post(
            process.env.CART_SERVICE_URL + "/api/cart/checkout",
            {
...
exports.cancelOrder = async (req, res) => {
  try {
    ...
    try {
      await axios.post(
        process.env.ORDER_SERVICE_URL + "/api/orders/cancel-order",
        {
          orderCode: orderCode,
          reason: cancelReason,
        }
